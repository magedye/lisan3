...
def test_purity_not_evaluated_if_missing_evidence():
    ...
    claim = models.SemanticClaim(id="c1", research_run_id="run1", contract_type="ROOT_CORE")
    score, rating, findings, flags = evaluate_methodological_purity(claim, db)
...
def test_purity_dictionary_first_flagged():
    ...
    hyp = models.Hypothesis(id="h1", research_run_id=run_id, created_at=now - timedelta(days=2))
    obs = models.ObservationArtifact(id="o1", research_run_id=run_id, occurrence_ref="ref1", created_at=now - timedelta(days=1))
    claim = models.SemanticClaim(id="c2", research_run_id=run_id, contract_type="ROOT_CORE")
    
    db.add_all([hyp, obs, claim])
    db.commit()
    
    score, rating, findings, flags = evaluate_methodological_purity(claim, db)
    dict_first_finding = next(f for f in findings if f["dimension"] == "DICTIONARY_FIRST")
...
def test_purity_blocks_admission():
    ...
    assert any("Missing required PASSED GateReport for 'PURITY_CHECK'" in r for r in result["reasons"])
