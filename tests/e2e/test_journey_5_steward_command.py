...
def test_journey_5_steward_command(page: Page, e2e_server: dict):
    ...
    # 1. No executable command is currently registered; unsupported is honest.
    ...
    expect(page.locator("#cmd-status")).to_contain_text("UNSUPPORTED", timeout=10000)

    # 2. Verify Audit Log recorded the Steward action
    page.click("button:has-text('View Audit Logs')")
    expect(page.locator(".log-actor").first).to_contain_text("STEWARD_COMMAND_SERVICE")
    expect(page.locator(".log-action").first).to_contain_text("REJECT_UNSUPPORTED")
    ...
    expect(page.locator("#cmd-error")).to_contain_text(
        "cannot bypass epistemic lifecycle", ignore_case=True, timeout=10000
    )
