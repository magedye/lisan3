...
| Research Run / Root Workspace / Hypothesis view | `/run/[id]` | `GET /runs/{run_id}/workspace` | Aggregate exposes evidence-bound research artifacts; canonicalization remains separate | Yes / Yes / Yes | Checkpoints do not grant authority. Research Judgments are visible when source isolation is clean; accepted state requires the canonicalization policy. | Updated under the 2026-09-06 simplification; regression evidence is recorded in the current task. |
